...
parser.add_argument('--model', default='gesturenet_with_different_LSTMs', help='Model name [default: gesturenet]')
...
parser.add_argument('--log_dir', default='/scratch/work/salamid1/trained_models/UBPG/hyperparameters/1LSTM', help='Log dir [default: log]')
...
NUMBER_OF_LSTM_LAYERS = FLAGS.LSTM_layers
...
TEST_DATASET = gesturenet_h5_dataset.GestureNetH5Dataset('{}/validation_files.txt'.format(DATASET), nframes=NUM_FRAME, batch_size=BATCH_SIZE, npoints=NUM_POINT, shuffle=False)
...
def train():
    with tf.Graph().as_default():
        with tf.device('/device:GPU:'+str(GPU_INDEX)):
            ...
            # Get model and loss
            pred, end_points = MODEL.get_model(pointclouds_pl, is_training_pl, num_classes=NUM_CLASSES, num_lstm_layers=NUMBER_OF_LSTM_LAYERS, bn_decay=bn_decay)
