...
parser.add_argument('--num_frame', type=int, default=20, help='Frame Number [default: 20]')
...
parser.add_argument('--early_stopping_patience', type=int, default=100, help='Stop the training if there is no improvements after this number of consequent epochs [default: 100]')
...
VALIDATION_DATASET = gesturenet_h5_dataset.GestureNetH5Dataset('{}/validation_files.txt'.format(DATASET), nframes=NUM_FRAME, batch_size=BATCH_SIZE, npoints=NUM_POINT, shuffle=False)
...
def train():
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            current_acc = eval_one_epoch(sess, ops, test_writer)
            ...
            if EARLY_STOPPING == 'True' and last_improvment > EARLY_STOPPING_PATIENCE:
                log_string("No improvement was observed after {} epochs.".format(last_improvment))
                log_string("The best model with the accuracy of {} on the validation set was saved at epoch {}.".format(best_acc, best_acc_epoch))
                sys.stdout.flush()
                break
...
def train_one_epoch(sess, ops, train_writer):
    ...
    cur_batch_data = np.zeros((BATCH_SIZE,NUM_FRAME,NUM_POINT,TRAIN_DATASET.num_channel()))
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    # Make sure batch data is of same size
    cur_batch_data = np.zeros((BATCH_SIZE,NUM_FRAME,NUM_POINT,VALIDATION_DATASET.num_channel()))
    cur_batch_label = np.zeros((BATCH_SIZE), dtype=np.int32)
    ...
    while VALIDATION_DATASET.has_next_batch():
        batch_data, batch_label = VALIDATION_DATASET.next_batch(augment=False)
        bsize = batch_data.shape[0]
    ...
    log_string('eval avg class acc: %f' % avg_class_acc)
    EPOCH_CNT += 1
