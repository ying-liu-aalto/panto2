...
def get_lstm_point_net_model(num_frame, l0_xyz, l0_points, batch_size, num_point, is_training, bn_decay):
    ...
    for frame_index in range(num_frame):
        ...
        l1_xyz, l1_points, l1_indices = pointnet_sa_module(point_net_input, l0_points, npoint=512, radius=0.2, nsample=32, mlp=[64,64,128], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='frame-layer1', use_nchw=True, reuse_weights=frame_index > 0)
        l2_xyz, l2_points, l2_indices = pointnet_sa_module(l1_xyz, l1_points, npoint=128, radius=0.4, nsample=64, mlp=[128,128,256], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='frame-layer2', reuse_weights=frame_index > 0)
        l3_xyz, l3_points, l3_indices = pointnet_sa_module(l2_xyz, l2_points, npoint=None, radius=None, nsample=None, mlp=[256,512,1024], mlp2=None, group_all=True, is_training=is_training, bn_decay=bn_decay, scope='frame-layer3', reuse_weights=frame_index > 0)
    ...
    net = tf.keras.layers.LSTM(32, return_sequences=True, dropout=0.4, recurrent_dropout=0.4)(net, scope='lstm2', training=is_training)
...
def get_model(point_cloud, is_training, num_classes, bn_decay=None):
    ...
    output_of_temporal_part = get_lstm_point_net_model(num_frame, l0_xyz, l0_points, batch_size, num_point, is_training, bn_decay)
    output_of_agg_part = get_aggregated_point_net_model(num_frame, l0_xyz, l0_points, batch_size, num_point, is_training, bn_decay)
    net = tf.concat([tf.expand_dims(output_of_temporal_part, 1), tf.expand_dims(output_of_agg_part, 1)], 1)
    net = tf_util.batch_norm_for_fc(net, is_training=is_training, scope='bn', bn_decay=bn_decay)
    net = tf_util.attention_3d_block(net)
    net = tf_util.fully_connected(net, num_classes, activation_fn=None, scope='global-fc1')
