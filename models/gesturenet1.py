...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    for frame_index in range(num_frame):
        ...
        input_image = tf.expand_dims(point_net_input, -1)
        # Point functions (MLP implemented as conv2d)
        net = tf_util.conv2d(input_image, 64, [1,3],
                             padding='VALID', stride=[1,1],
                             bn=True, is_training=is_training,
                             scope='conv1', bn_decay=bn_decay,
                             reuse_weights=frame_index > 0)
    ...
    net = tf.keras.layers.LSTM(36, dropout=0.4, recurrent_dropout=0.4)(net, scope='lstm2', training=is_training)
