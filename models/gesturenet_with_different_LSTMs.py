...
def get_model(point_cloud, is_training, num_classes, num_lstm_layers=1, bn_decay=None):
    ...
    end_points['l2_xyz'] = None
    end_points['l2_points_before_pooling'] = None
    end_points['l2_indices'] = None
    ...
    for frame_index in range(num_frame):
        ...
        l2_xyz, l2_points, l2_indices, l2_points_before_pooling = pointnet_sa_module(l1_xyz, l1_points, npoint=128, radius=0.4, nsample=64, mlp=[128,128,256], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='frame-layer2', reuse_weights=frame_index > 0)
        l3_xyz, l3_points, l3_indices, _ = pointnet_sa_module(l2_xyz, l2_points, npoint=None, radius=None, nsample=None, mlp=[256,512,1024], mlp2=None, group_all=True, is_training=is_training, bn_decay=bn_decay, scope='frame-layer3', reuse_weights=frame_index > 0)
        if end_points['l2_xyz'] is None:
            end_points['l2_xyz'] = tf.expand_dims(l2_xyz, 1)
            end_points['l2_points_before_pooling'] = tf.expand_dims(l2_points_before_pooling, 1)
            end_points['l2_indices'] = tf.expand_dims(l2_indices, 1)
        else:
            end_points['l2_xyz'] = tf.concat([end_points['l2_xyz'], tf.expand_dims(l2_xyz, 1)], 1)
            end_points['l2_points_before_pooling'] = tf.concat([end_points['l2_points_before_pooling'], tf.expand_dims(l2_points_before_pooling, 1)], 1)
            end_points['l2_indices'] = tf.concat([end_points['l2_indices'], tf.expand_dims(l2_indices, 1)], 1)
    ...
    for i in range(num_lstm_layers):
        if i < num_lstm_layers - 1:
            net = tf.keras.layers.LSTM(32, return_sequences=True, dropout=0.4, recurrent_dropout=0.4)(net, scope='lstm{}'.format(i + 1), training=is_training)
        else:
            net = tf.keras.layers.LSTM(32, dropout=0.4, recurrent_dropout=0.4)(net, scope='lstm{}'.format(i + 1), training=is_training)
